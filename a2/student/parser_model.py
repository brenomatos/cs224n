...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        w = nn.init.xavier_uniform_(torch.empty(self.embed_size * n_features, hidden_size))
        self.embed_to_hidden_weight = nn.Parameter(w) #W

        b1 = nn.init.uniform_(torch.empty(hidden_size,))
        self.embed_to_hidden_bias = nn.Parameter(b1) #b1

        u = nn.init.xavier_uniform_(torch.empty(hidden_size,n_classes))
        self.hidden_to_logits_weight = nn.Parameter(u)

        b2 = nn.init.uniform_(torch.empty(n_classes,))
        self.hidden_to_logits_bias = nn.Parameter(b2) #b2
    ...
    def embedding_lookup(self, w):
        ...
        for features in w:
            ...
            for index in features:
                emb.append(self.embeddings[index])
            x.append(torch.cat(emb))
    ...
    def forward(self, w):
        ...
        h = nn.functional.relu(embeddings @ self.embed_to_hidden_weight + self.embed_to_hidden_bias)
        h = self.dropout(h)
